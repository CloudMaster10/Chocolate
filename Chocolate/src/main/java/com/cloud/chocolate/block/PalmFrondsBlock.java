...
public class PalmFrondsBlock extends Block implements net.minecraftforge.common.IShearable {
	...
	public PalmFrondsBlock(Block.Properties properties) {
		super(properties);
	...
	public void randomTick(BlockState state, ServerWorld worldIn, BlockPos pos, Random random) {
		...
		if((!state.get(PERSISTENT) && state.get(DISTANCE) == 7) || (newState.get(DISTANCE) > state.get(DISTANCE))) {
	...
	private static int getDistance(BlockState neighbor) {
		if (BlockTags.LOGS.contains(neighbor.getBlock())) {
			...
			return neighbor.getBlock() instanceof PalmFrondsBlock ? neighbor.get(DISTANCE) : 7;
		}
	}
