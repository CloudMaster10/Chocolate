package com.cloud.chocolate.entity.item;
...
public class ModBoatEntity extends BoatEntity
{
	...
	private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.createKey(ModBoatEntity.class,DataSerializers.VARINT);
	private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.createKey(ModBoatEntity.class,DataSerializers.FLOAT);
	...
	private static final DataParameter<Boolean> PADDLE_LEFT = EntityDataManager.createKey(ModBoatEntity.class,DataSerializers.BOOLEAN);
	private static final DataParameter<Boolean> PADDLE_RIGHT = EntityDataManager.createKey(ModBoatEntity.class,DataSerializers.BOOLEAN);
	private static final DataParameter<Integer> ROCKING_TICKS = EntityDataManager.createKey(ModBoatEntity.class,DataSerializers.VARINT);
	...
	public ModBoatEntity(EntityType<? extends BoatEntity> entityTypeIn, World worldIn)
	{
	...
	protected void registerData()
	{
		...
		this.dataManager.register(BOAT_TYPE, ModBoatEntity.Type.PALM.ordinal());
		this.dataManager.register(PADDLE_LEFT, false);
		this.dataManager.register(PADDLE_RIGHT, false);
	...
	{
		if (entityIn instanceof ModBoatEntity)
		{
	...
	public void tick()
	{
		...
		if (this.status != ModBoatEntity.Status.UNDER_WATER && this.status != ModBoatEntity.Status.UNDER_FLOWING_WATER)
		{
	...
	{
		this.dataManager.set(PADDLE_LEFT, left);
		this.dataManager.set(PADDLE_RIGHT, right);
	}
	...
	private ModBoatEntity.Status getBoatStatus()
	{
		ModBoatEntity.Status ModBoatEntity$status = this.getUnderwaterStatus();
		if (ModBoatEntity$status != null)
		{
		...
		{
			return ModBoatEntity.Status.IN_WATER;
		}
		else
		{
			...
			{
				this.boatGlide = f;
				return ModBoatEntity.Status.ON_LAND;
			}
			else
			{
				return ModBoatEntity.Status.IN_AIR;
			}
		}
	}
	...
	private void updateMotion()
	{
		...
		double d2 = 0.0D;
		this.momentum = 0.05F;
		if (this.previousStatus == ModBoatEntity.Status.IN_AIR && this.status != ModBoatEntity.Status.IN_AIR && this.status != ModBoatEntity.Status.ON_LAND)
		{
			...
			this.lastYd = 0.0D;
			this.status = ModBoatEntity.Status.IN_WATER;
		}
		else
		{
			if (this.status == ModBoatEntity.Status.IN_WATER)
			{
				d2 = (this.waterLevel - this.getPosY()) / (double) this.getHeight();
				this.momentum = 0.9F;
			}
			else if (this.status == ModBoatEntity.Status.UNDER_FLOWING_WATER)
			{
				d1 = -7.0E-4D;
				this.momentum = 0.9F;
			}
			else if (this.status == ModBoatEntity.Status.UNDER_WATER)
			{
				d2 = (double) 0.01F;
				this.momentum = 0.45F;
			}
			else if (this.status == ModBoatEntity.Status.IN_AIR)
			{
				this.momentum = 0.9F;
			}
			else if (this.status == ModBoatEntity.Status.ON_LAND)
			{
	...
	protected void readAdditional(CompoundNBT compound)
	{
		...
		{
			this.setModBoatType(ModBoatEntity.Type.getTypeFromString(compound.getString("Type")));
		}
	}
	...
	{
		return this.dataManager.<Boolean>get(side == 0 ? PADDLE_LEFT : PADDLE_RIGHT) && this.getControllingPassenger() != null;
	}
	...
	public void setModBoatType(ModBoatEntity.Type boatType) {
		this.dataManager.set(BOAT_TYPE, boatType.ordinal());
	}

	public ModBoatEntity.Type getModBoatType()
	{
		return ModBoatEntity.Type.byId(this.dataManager.get(BOAT_TYPE));
	}
	...
	*/
	...
	public static enum Type
	{
		...
		{
			ModBoatEntity.Type[] aModBoatEntity$type = values();
		...
		public static ModBoatEntity.Type getTypeFromString(String nameIn)
		{
			...
			for (int i = 0; i < aModBoatEntity$type.length; ++i)
			{
				if (aModBoatEntity$type[i].getName().equals(nameIn))
				{
					return aModBoatEntity$type[i];
				}
			}

			return aModBoatEntity$type[0];
		}
	}
}